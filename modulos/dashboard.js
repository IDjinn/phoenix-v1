...
module.exports = (client) => {
	...
	if (client.servidorConfig.dashboard.enabled !== 'true') return client.log('log', 'Dashboard disabled', 'INFO');

	const oauth2 = client.user.id == '503239059775422491' ? client.servidorConfig.dashboard.oauth : client.servidorConfig.dashboard.oauthBeta
	const domain = client.user.id == '503239059775422491' ? client.servidorConfig.dashboard.domain : client.servidorConfig.dashboard.domainBeta
	const secure = client.user.id == '503239059775422491' ? client.servidorConfig.dashboard.secure : client.servidorConfig.dashboard.secureBeta
	...
	See servidorConfig.js.example to set these up.
	*/
	...
	client.callbackURL = `${protocol}${domain}/callback`;
	...
	app.use(session({
		secret: client.servidorConfig.dashboard.sessionSecret,
		resave: false,
	...
	*/
	...
	function checkAdmin(req, res, next) {
		if (req.isAuthenticated() && req.user.id === client.servidorConfig.ownerID) return next();
	...
	fs.readFile(`${process.cwd()}${path.sep}dashboard${path.sep}public${path.sep}PRIVACY.md`, function(err, data) {
		...
		privacyMD = data.toString().replace(/\{\{botName\}\}/g, client.user.username).replace(/\{\{email\}\}/g, client.servidorConfig.dashboard.legalTemplates.contactEmail);
		if (secure !== 'true') {
	...
	fs.readFile(`${process.cwd()}${path.sep}dashboard${path.sep}public${path.sep}TERMS.md`, function(err, data) {
		...
		termsMD = data.toString().replace(/\{\{botName\}\}/g, client.user.username).replace(/\{\{email\}\}/g, client.servidorConfig.dashboard.legalTemplates.contactEmail);
	});
	...
	app.get('/stats', (req, res) => {
		if (client.servidorConfig.dashboard.protectStats === 'true') {
			cAuth(req, res);
		}
	...
	app.get('/legal', function (req, res) {
		...
		res.render(path.resolve(`${templateDir}${path.sep}legal.ejs`), {
			...
			edited: client.servidorConfig.dashboard.legalTemplates.lastEdited
		});*/
	...
	app.get('/add/:guildID', checkAuth, (req, res) => {
		...
		var invitePerm = client.servidorConfig.dashboard.invitePerm;
	...
	app.post('/manage/:guildID/:page', checkAuth, async (req, res) => {
		const guild = client.guilds.get(req.params.guildID);
		const page = req.params.page || 'geral'
		if (!guild) return res.status(404);
		const isManaged = guild && !!guild.member(req.user.id) ? guild.member(req.user.id).permissions.has('MANAGE_GUILD') : false;
		if (req.user.id === client.servidorConfig.ownerID) {
		...
		servidor = await client.settings.get(guild.id, settings)
		delete client.servidores[guild.id]
		delete guild.dados
		await Servidor.findOneAndDelete({id: guild.id}, function (err,sucesso) {
        ...
        const s = new Servidor({
            ...
            comandos: servidor.comandos,
            anunciar: servidor.anunciar,
	        logs: servidor.logs,
            niveis: servidor.niveis,
            verificar: servidor.verificar,
            punicao: servidor.punicao,
            contadorbot: servidor.contadorbot,
            contadormembro: servidor.contadormembro,
            adm: servidor.adm,
            mod: servidor.mod,
            inicial: servidor.inicial,
			verificado: servidor.verificado,
	        autowarn: servidor.autowarn,
            antiraid: servidor.antiraid,
            antispamemoji: servidor.antispamemoji,
            antispaminvite: servidor.antispaminvite,
            antispammention: servidor.antispammention,
            blacklink: servidor.blacklink,
            whitelink: servidor.whitelink,
            antispamcaps: servidor.antispamcaps,
            lockserver: servidor.lockserver,
            serverislocked: servidor.serverislocked,
            textdup: servidor.textdup,
            singular: servidor.singular,
            plural: servidor.plural,
		...
		await s.save()
		res.redirect(`/manage/${req.params.guildID}/${page}`);
	});

	app.get('/manage/:guildID/:page', checkAuth, (req, res) => {
		const guild = client.guilds.get(req.params.guildID);
		const page = req.params.page || 'geral'
		if (!guild) return res.status(404);
		const isManaged = guild && !!guild.member(req.user.id) ? guild.member(req.user.id).permissions.has('MANAGE_GUILD') : false;
		...
		res.render(path.resolve(`${templateDir}${path.sep}manage/${page}.ejs`), {
			bot: client,
			guild: guild,
			user: req.user,
			auth: true
	...
	app.get('/reset/:guildID', checkAuth, async (req, res) => {
		...
		client.settings.set(guild.id, client.servidorConfig.defaultSettings);
		res.redirect(`/manage/${req.params.guildID}/geral`);
	});
	...
	let pota = process.env.PORT || client.servidorConfig.dashboard.port
	client.site = app.listen(pota, function() {
		client.log('log', `Dashboard running on port ${pota}`, 'INFO');
	}).on('error', (err) => {
